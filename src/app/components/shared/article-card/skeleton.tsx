  ...
  if (type === "overlay") {
    return (
      <Card className="relative h-80 overflow-hidden rounded-lg animate-pulse">
        <Skeleton
          className={`absolute inset-0 w-full h-full ${baseSkeleton}`}
        />
  ...
  if (type === "bottomOverlay") {
    return (
      <Card className="relative h-80 rounded-lg animate-pulse p-0">
        <Skeleton className={`w-full h-full rounded-lg ${baseSkeleton}`} />
        <CardContent className="absolute -bottom-[20%] left-1/2 -translate-x-1/2 w-[85%] p-4 rounded-lg flex flex-col gap-2 text-center bg-gray-300 min-h-36">
